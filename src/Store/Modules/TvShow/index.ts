...
import { TvShow, TvShowCalendar, TvShowSearch } from "../../../Domain/Models/TvShow";
import { calendar, ICalendar, IRecentlyAdded, recentlyAdded } from "../Shared";

export interface ITvShowState {
    recentlyAddedTvShows: IRecentlyAdded<TvShow, TvShowSearch>;
...
export const tvShowState: ITvShowState = {
    recentlyAddedTvShows: recentlyAdded<TvShow, TvShowSearch>(
        (params, filters) => {
        ...
        (title) => {
            const apiClient = ApiClientProvider.getClient();
            return apiClient.v1.tvShows.searchTvShowByQuery(title);
        }
    ),
